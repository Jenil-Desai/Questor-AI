...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        const res = await axios.get('http://localhost:8000/api/v1/question-papers', {
          headers: {
        ...
        console.error('Error fetching question paper history', err);
  ...
  const handleSubmit = async (e) => {
    ...
    const requestBody = {
      id: uuidv4(),
      material: uploadedMaterial.name,
      difficulty,
      questionType: {
        MCQ: {
  ...
  const downloadQuestionPaper = async (paperId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:8000/api/v1/question-paper/${paperId}/download`, {
        responseType: 'blob',
        headers: {
          Authorization: `Bearer ${token}`,
        },
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full p-3 bg-gray-700 border border-gray-600 rounded transition duration-200 focus:ring-2 focus:ring-blue-400 focus:outline-none"
                />
              </div>
                ...
                <div className="mb-8" key={index}>
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      type="number"
                      value={section.count}
                      onChange={(e) => section.setCount(e.target.value)}
                      className="w-full p-3 bg-gray-700 border border-gray-600 rounded transition duration-200 focus:ring-2 focus:ring-blue-400 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={section.marks}
                      onChange={(e) => section.setMarks(e.target.value)}
                      className="w-full p-3 bg-gray-700 border border-gray-600 rounded transition duration-200 focus:ring-2 focus:ring-blue-400 focus:outline-none"
                    />
                  </div>
                </div>
            ...
            <div className="space-y-4">
              {history.map((paper) => (
                <div key={paper.id} className="bg-gray-900 p-6 rounded-lg shadow-md">
                  <p><strong>Material:</strong> {paper.material}</p>
