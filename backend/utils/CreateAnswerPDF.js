...
export default async function createAnswerPDF(questionPaper) {
  const dir = "./pdfs";
  ...
  const filePath = path.join(dir, `${questionPaper.title.replace(/\s+/g, "_")}-Answer-Paper-Questro-AI-${questionPaper.id}.pdf`);
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(20).text(`${questionPaper.title} - Answer Paper`, { align: "center", underline: true });
    doc.moveDown();
    ...
    questionPaper.questions.forEach((q, index) => {
      ...
      if (q.type === "MCQ") {
        q.options.forEach((option) => {
          let color = option == q.answer ? "green" : "black";
          doc.fontSize(12).fillColor(color).text(` · ${option}`);
        });
      }
      q.type != "MCQ" ? doc.fontSize(10).text(`Ans. ${q.answer}`) : null;
      doc.moveDown();
    });
